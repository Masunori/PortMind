...
from app.domain.repository import SignalRelationship
...
from app.repositories import get_planning_cycle_repository
from app.repositories.contracts import PlanningCycleRepository
...
def save_cycle(cycle: PlanningCycle, repository: PlanningCycleRepository | None = None,
               *, expected_version: int | None = None) -> PlanningCycle:
    """Persist a workflow snapshot without treating copied metrics as authoritative input."""
    return (repository or get_planning_cycle_repository()).save(cycle, expected_version=expected_version)


def get_cycle(cycle_id: str, repository: PlanningCycleRepository | None = None) -> PlanningCycle | None:
    return (repository or get_planning_cycle_repository()).get(cycle_id)


def list_cycles(repository: PlanningCycleRepository | None = None) -> list[PlanningCycle]:
    return list((repository or get_planning_cycle_repository()).list(limit=1000).items)
...
def _load_risk_candidates(context_version: str, starts_at: datetime, ends_at: datetime,
                          repository: PlanningCycleRepository):
    """Load eligible stored signals and their trusted normalized disruptions."""
    eligible=[]
    for item in repository.risk_candidates(context_version):
        window=TemporalWindow.model_validate(item.temporal_window)
        if _window_overlaps(window,starts_at,ends_at):eligible.append((item,window))
    return eligible,repository.relationships()


def _validate_selected_relationships(selected: set[str], relationships: list[SignalRelationship]) -> None:
...
class PlanningService:
    ...
    def __init__(self, risk_provider: RiskProvider, planner_provider: PlannerProvider,
                 repository: PlanningCycleRepository | None = None):
        self._risk_provider = risk_provider
        self._planner_provider = planner_provider
        self._repository = repository or get_planning_cycle_repository()
    ...
    async def generate_scenarios(self, *, gateway: ClientGateway,
        ...
        eligible, relationships = _load_risk_candidates(context.context_version, starts_at, ends_at,self._repository)
        eligible_ids = {entity_id for item,_ in eligible for entity_id in item.entity_ids}
        eligible_version_ids = [item.id for item,_ in eligible]
        grounded_scope = [GenerationEntity(entity_id=item.entity_id,entity_type=item.entity_type,display_name=item.display_name) for item in self._repository.grounded_entities(eligible_version_ids,eligible_ids,context.context_version)]
        ...
        candidates = [RiskSignalCandidate(signal_version_id=version.id,
            ...
            entity_ids=list(version.entity_ids)) for version, window in eligible]
        ...
        eligible_by_id = {version.id: version for version, _ in eligible}
        ...
        for proposal in sorted(proposals, key=lambda item: item.proposal_id):
            ...
            for version_id in proposal.selected_signal_version_ids:
                version = eligible_by_id[version_id]
                stored = version.normalized_disruption
    ...
    def compose_scenario(self, cycle: PlanningCycle,
        ...
        relationships = self._repository.relationships()
