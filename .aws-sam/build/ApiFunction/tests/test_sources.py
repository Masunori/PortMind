...
def test_scheduler_retains_unexpected_processing_failure(
    ...
    monkeypatch.setattr(scheduler_service, "get_due_sources", lambda _now, _repository=None: [source])
    monkeypatch.setattr(
        scheduler_service, "record_source_run",
        lambda source_id, error=None, repository=None: recorded.append((source_id, error)),
    )
