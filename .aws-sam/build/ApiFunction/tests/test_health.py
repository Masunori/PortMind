...
from app.repositories.factory import Storage
...
def test_storage_health_exposes_selected_backend(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(main, "get_storage", lambda: HealthyStorage())

    assert main.storage_health() == {
        "status": "ok", "storage": "ok", "backend": "postgres"
    }


def test_storage_health_translates_unavailability(monkeypatch: pytest.MonkeyPatch) -> None:
    class BrokenStorage:
        backend = "postgres"

        def check_health(self) -> None:
            raise UnavailableError("secret vendor detail")

    monkeypatch.setattr(main, "get_storage", lambda: BrokenStorage())
    with pytest.raises(HTTPException) as caught:
        main.storage_health()
    assert caught.value.status_code == 503
    assert caught.value.detail == "storage unavailable"
