...
def store_evidence(values: EvidenceCreate) -> tuple[Evidence, bool]:
    ...
    if values.content is not None and len(values.content.encode("utf-8")) > MAX_EVIDENCE_CONTENT_BYTES:
        raise ValidationError("evidence content exceeds 256 KiB")
    now = datetime.now(timezone.utc); digest = _digest(values)
    with SessionLocal.begin() as session:
        if session.get(DataSourceRecord, values.source_id) is None: raise NotFoundError("Source not found")
...
def set_archived(evidence_id: str, archived: bool) -> Evidence:
    with SessionLocal.begin() as session:
        ...
        if record is None: raise NotFoundError("Evidence not found")
...
def duplicate_deletion_preview(evidence_id: str) -> DuplicateDeletionPreview:
    ...
    with SessionLocal() as session:
        ...
        if canonical is None: raise NotFoundError("Evidence not found")
        if canonical.duplicate_of_id is not None:
            raise ValidationError("Batch cleanup must target canonical evidence")
...
def remove_raw_content(evidence_id: str) -> Evidence:
    ...
    if not impact.can_remove_raw_content: raise ConflictError("Evidence is on legal hold")
    with SessionLocal.begin() as session:
        record = session.get(EvidenceRecord, evidence_id)
...
def update_evidence(evidence_id: str, values: EvidenceUpdate) -> Evidence:
    ...
    if impact.protected_by:
        raise ConflictError("Evidence linked to an audit workflow cannot be edited")
...
def delete_evidence(evidence_id: str) -> None:
    ...
    if not impact.can_delete_permanently:
        raise ConflictError("Evidence linked to an audit workflow cannot be deleted")
    with SessionLocal.begin() as session:
        record = session.get(EvidenceRecord, evidence_id)
        if record is None:
            raise NotFoundError("Evidence not found")
...
class PostgresEvidenceRepository:
    """Implement the evidence contract using transactional SQLAlchemy operations."""
    @_database_errors
    def store(self, values: EvidenceCreate) -> tuple[Evidence, bool]: return store_evidence(values)
    @_database_errors
    def list(self, *, archived: bool | None = False, kind: EvidenceKind | None = None,
        ...
        items = list_evidence(archived=archived, kind=kind, include_duplicates=include_duplicates,
                              limit=limit + 1, offset=offset)
        return Page(tuple(items[:limit]), encode_offset(offset, limit) if len(items) > limit else None)
    @_database_errors
    def get(self, evidence_id: str) -> Evidence | None: return get_evidence(evidence_id)
    @_database_errors
    def has_signal(self, evidence_id: str) -> bool: return evidence_has_signal(evidence_id)
    @_database_errors
    def set_archived(self, evidence_id: str, archived: bool) -> Evidence: return set_archived(evidence_id, archived)
    @_database_errors
    def deletion_impact(self, evidence_id: str) -> DeletionImpact: return deletion_impact(evidence_id)
    @_database_errors
    def duplicate_deletion_preview(self, evidence_id: str) -> DuplicateDeletionPreview: return duplicate_deletion_preview(evidence_id)
    @_database_errors
    def delete_unprotected_duplicates(self, evidence_id: str, *, delete_canonical: bool = False) -> DuplicateDeletionResult: return delete_unprotected_duplicates(evidence_id, delete_canonical=delete_canonical)
    @_database_errors
    def remove_raw_content(self, evidence_id: str) -> Evidence: return remove_raw_content(evidence_id)
    @_database_errors
    def update(self, evidence_id: str, values: EvidenceUpdate) -> Evidence: return update_evidence(evidence_id, values)
    @_database_errors
    def delete(self, evidence_id: str) -> None: delete_evidence(evidence_id)
